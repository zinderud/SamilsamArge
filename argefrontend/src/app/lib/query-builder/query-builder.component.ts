...
export class QueryBuilderComponent implements OnInit, OnChanges, ControlValueAccessor, Validator {
  ...
  public defaultOperatorMap: { [key: string]: string[] } = {
    string: ['esit', 'esit_olmayan', 'iceren', 'icinde', 'icinde_olmayan', 'ile_baslayan', 'ile_baslamayan', 'ile_biten', 'ile_bitmeyen', 'bos_olan', 'bos_olmayan'],
    number: ['esit', 'esit_olmayan', 'buyuk', 'buyuk_esit', 'kucuk', 'kucuk_esit'],
    integer: ['esit', 'esit_olmayan', 'buyuk', 'buyuk_esit', 'kucuk', 'kucuk_esit'],
    time: ['esit', 'esit_olmayan', 'buyuk', 'buyuk_esit', 'kucuk', 'kucuk_esit'],
    date: [ 'buyuk', 'buyuk_esit', 'kucuk', 'kucuk_esit'],
    category: ['esit', 'esit_olmayan', 'icinde', 'icinde_olmayan'],
    long: ['esit', 'esit_olmayan', 'icinde', 'icinde_olmayan'],
    boolean: ['esit']
  };
  @Input() disabled: boolean;
  @Input() data: any = { condition: 'and', rules: []  };
  ...
  @Input() emptyMessage: string = 'Bir kural kümesi boş olamaz. Lütfen bir kural ekleyin veya hepsini birlikte kaldırın.';
  ...
  private defaultTemplateTypes: string[] = [
    'string', 'number','integer', 'time', 'date', 'category','long', 'boolean', 'multiselect'];
  private defaultPersistValueTypes: string[] = [
    'string', 'number','integer', 'time', 'date','long', 'boolean'];
  ...
  ngOnChanges(changes: SimpleChanges) {
    ...
    if (type === 'object') {
      ...
      throw new Error(`Beklenen 'config' geçerli bir nesne olmalı, bunun yerine ${type} alındı.`);
    }
  }
  ...
  validate(control: AbstractControl): ValidationErrors | null {
    ...
    if (!this.config.allowEmptyRulesets && this.checkEmptyRuleInRuleset(this.data)) {
      errors.empty = 'Boş kural kümelerine izin verilmez. ';
      hasErrors = true;
    }
    ...
    if (type) {
      ...
      if (queryInput) {
        ...
        if (this.defaultTemplateTypes.indexOf(type) === -1) {
          console.warn(`Şablon bulunamadı:: ${type}`);
        }
        return null;
      }
    }
  ...
  getOperators(field: string): string[] {
    ...
    if (fieldObject && fieldObject.operators) {
      ...
      if (operators.length === 0) {
        console.warn(
          `Alan için operatör bulunamadı '${field}'   type ${fieldObject.type}. ` +
          `Lütfen sahada bir 'operatörler' özelliği tanımlayın veya bunu düzeltmek için 'operatorMap' bağlamasını kullanın.`);
      }
      if (fieldObject.nullable) {
        operators = operators.concat(['is null', 'is not null']);
      }
    } else {
      console.warn(`Alanda 'type' özelliği bulunamadı: '${field}'`);
    }
  ...
  getDefaultOperator(field: Field): string {
    if (field && field.defaultOperator !== undefined) {
      ...
      if (operators && operators.length) {
        ...
        console.warn(`Alan için operatör bulunamadı '${field.value}'. ` +
          `A Alan yapılandırmasında 'defaultOperator' belirtilmez. Operatör değeri varsayılan olarak null`);
        return null;
      }
    }
  }
