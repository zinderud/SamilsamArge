...
namespace argebackend.Controllers
{
    ...
    public class PersonController : BaseController
    {
        ...
        public async Task<IActionResult> Retrieve([FromRoute] long id)
        {
            ...
            var result = await _PersonService.RetrieveAsync(id);
            if (!result.Succeeded)
                return BadRequest(result.Errors);
        ...
        /*   [Authorize(Roles = "Admin")] */
        [AllowAnonymous]
        public async Task<IActionResult> Update([FromRoute] long id, [FromBody] Person model)
        {
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
        ...
        public async Task<IActionResult> List([FromQuery] GetListViewModel<BaseFilter> listModel)
        {
            ...
            var result = await _PersonService.ListAsync(listModel);
            if (!result.Succeeded)
                return BadRequest(result.Errors);
        ...
        public async Task<IActionResult> Count(BaseFilter filter)
        {
            ...
            var result = await _PersonService.CountAsync(filter);
            if (!result.Succeeded)
                return BadRequest(result.Errors);
